...
# fork https://raw.githubusercontent.com/a3165458/Titan-Network/main/duokai.sh
...
for ((i = 1; i <= container_count; i++)); do
  ...
  # 运行容器，并设置重启策略为always
  container_id=$(docker run -d --restart always -v "$storage_path:/root/.titanedge/storage" --name "titan$i" --net=host nezha123/titan-edge:latest)
